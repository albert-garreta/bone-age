...
def remove_gray(img_hsv):
    mask = cv2.inRange(img_hsv, (0, 50, 0), (255, 255, 255))
    show(mask)
...
def apply_max_saturation_and_value(img_hsv):
    ...
    s[::] = 255  # 255*np.ones(s.shape)
    #v[::] = 100  # 255*np.ones(v.shape)
    return cv2.merge((h, s, v))

if __name__ == "__main__":
    dir = "data/data_tagged"
    imgs = os.listdir(dir)
    for img in imgs[10:15]:
        try:
            ...
            for color, bounds in HSV_color_bounds.items():
                ...
                hand_bgr_copy = deepcopy(hand_hsv)
                ...
                hand_gray = cv2.cvtColor(hand_bgr_copy, cv2.COLOR_HSV2BGR)
                hand_gray = cv2.cvtColor(hand_gray, cv2.COLOR_BGR2GRAY)
